...
class SupplierValidator extends BaseValidator
{
    ...
    protected function performValidation(object $dto): void
    {
        ...
        $this->validateRequired($dto->document, 'CNPJ é obrigatório');
        $this->validateLength($dto->document, 14, 18, 'CNPJ inválido');

        if ($this->repository->existsByDocument($dto->document, $dto->id)) {
            $this->errors[] = 'CNPJ já está em uso';
        }
        ...
        $this->validateRequired($dto->contact, 'Contato é obrigatório');
        $this->validateLength($dto->contact, 2, 100, 'Contato deve ter entre 2 e 100 caracteres');
    }
}
